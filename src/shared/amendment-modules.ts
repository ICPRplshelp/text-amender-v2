...
const align: AmendmentModule = {
  ...
  operation: (text) => {
    function looksLikeLatex(input: string): boolean {
      if (
        /\\(frac|sqrt|sum|int|mathbf|mathrm|begin|end|alpha|beta|gamma|hvec|nabla|del)/.test(
          input,
        )
      ) {
        return true;
    ...
    const round_one = text
      .split("\n")
      .map((v) => (looksLikeLatex(v) ? fixOneLineOfBadNotation(v) : v))
      .join("\n\n");
    return round_one;
  },
};
...
const fixUnicodeEquations: AmendmentModule = {
  ...
  description:
    "Type an equation in MS Word (keep the mode on UnicodeMath), and copy it to here. This converts it into a form that Google's search can read. Not all operators are supported, so only use elementary arithmetic, complex numbers, and the choose operator",
  category: AmendmentCategories.WordEquations,
  operation: (text) => {
    return text
      .replaceAll("⋅", "*")
...
const pandocMarkdownToHTML: AmendmentModule = {
  ...
  description:
    "Takes Markdown text, and converts and Pandoc Markdown DIVs into HTML DIVs.",
...
const fakeListToList: AmendmentModule = {
  ...
  description:
    'Did someone try to write a list by using dashes, but forgot to add newlines before? Does some text look like this: "-Point 1 -Point 2- Point 3-Point 4?" This solves it.',
  inputType: "Text",
  category: AmendmentCategories.Strings,
  operation: (text) => {
    const pattern = /-/g;
...
const noLaTeXComments: AmendmentModule = {
  ...
  repr: "n-lt-c",
  description:
    "Removes all LaTeX comments (% and everything after, not including \\%). Note: may not work with verbatim.",
  inputType: "Text",
  category: AmendmentCategories.Strings,
  operation: (text) => {
    const sentinel =
      "accoあいうえおrdin光gto光jikosdrgkjKFHLUKRJSEFJIUOEFHHOLGFJU:SWJE";
    return text
      .split("\n")
      .map((line) =>
        line
...
const pdfNumberedListToText: AmendmentModule = {
  ...
  category: AmendmentCategories.Strings,
  description:
    "Removes all newlines if the line doesn't start with this regex: $\\d+\\.",
  operation: (text) => {
...
const softWrapper: AmendmentModule = {
  ...
  description:
    "Removes all lone newline characters. Do not modify text in markdown source code blocks.",
  operation: (text) => {
...
const plusMinus: AmendmentModule = {
  ...
  repr: "plus-minus",
  description:
    "Splits up equations with the plus minus sign into a complex number. - is the real component, and + is the imaginary component (it's like this for readability from left to right).",
  category: AmendmentCategories.WordEquations,
...
const toWindowsPath: AmendmentModule = {
  ...
  repr: "windows-path",
  description:
    'text.replace("/", R"\\"); also renames /c and /mnt/c for bash / wsl users',
  category: AmendmentCategories.Paths,
  inputType: "Text",
  operation: (text) => {
...
const toMathAM: AmendmentModule = {
  ...
  repr: "to-math",
  description:
    "Converts a math-like expression (in UnicodeMath - copied from MS Word) to code",
  category: AmendmentCategories.WordEquations,
  inputType: "Equation",
  operation: toMath,
};
...
*/
const transposeCSV: AmendmentModule = {
  name: "Transpose CSV",
  repr: "transpose-csv",
  description: "Transposes a CSV, if it is valid",
  ...
  operation: (text) => {
    ...
    const parseCSV = (str: string) => {
      ...
      return rows.map((row) => {
        ...
        // 2. Content between commas: ([^,]+)
        const regex =
          /"([^"]*(?:""[^"]*)*)"|([^,]+)|(?<=,)(?=,)|(?<=^)(?=,)|(?<=,)(?=$)/g;
        const matches = [];
        let m;
        while ((m = regex.exec(row)) !== null) {
          matches.push(m[1] ? m[1].replace(/""/g, '"') : m[2] || "");
        }
        return matches;
      });
    };

    // Step 1: Parse string to 2D Array
    const data = parseCSV(csvString);

    // Step 2: Transpose
    const transposed = transpose(data);

    // Step 3: Convert back to CSV string (adding quotes back where needed)
...
const transposeMatrix: AmendmentModule = {
  ...
  description:
    "Transposes a Microsoft Word Equation Editor Matrix. You may paste the entire Matrix, but the brackets for the matrix may not be (circular parenthesis), and the matrix may not contain nested matrices. The Matrix will always be output inside [square brackets].",
  category: AmendmentCategories.WordEquations,
  inputType: "Equation",
  operation: (text) => {
...
const wordMatrixToCode: AmendmentModule = {
  ...
  repr: "matrix-to-code",
  description:
    "Converts a Matrix in MS Word Format to code. You may paste the entire Matrix, but the brackets for the matrix may not be (circular parenthesis), and the matrix may not contain nested matrices",
  category: AmendmentCategories.WordEquations,
  inputType: "Equation",
  operation: (text) => {
    /*
    ...
    const matrixText = text.slice(firstBracket + 1, lastBracket);
    console.log(matrixText);
    const mtmt = matrixText
      .split("@")
      .map((line) => line.split("&").map(toMath));
...
const tsvToCsv: AmendmentModule = {
  ...
  repr: "tsv-to-csv",
  description:
    "Converts Tab-Seperated-Value files into CSV files. This assumes that no cell contains commas.",
  category: AmendmentCategories.CSV,
  operation: (text) => {
    return text.replaceAll("\t", ",");
  },
...
const csvToTsv: AmendmentModule = {
  ...
  repr: "csv-to-tsv",
  description:
    "Converts Comma-Seperated-Value files into Tab-separate-value strings. This assumes that no cell contains commas.",
  category: AmendmentCategories.CSV,
  operation: (text) => {
    return text.replaceAll(",", "\t");
  },
...
const tsvToJsonKeysBlankNull: AmendmentModule = {
  ...
  repr: "tsv-to-json",
  description:
    "Converts a specific TSV format into JSON. First column are the keys, second column are the values. The keys will always be treated as strings. Number conversions will happen if possible. If the string is TRUE or FALSE (case-insensitive), it will be converted to a boolean. Blank values will be set to null. All keys and values are trimmed before processing.",
  category: AmendmentCategories.CSV,
  operation: (text) => {
...
const csvToJsonKeys: AmendmentModule = {
  ...
  description:
    "Converts a specific CSV format into JSON. First column are the keys, second column are the values. Everything will remain a string.",
  category: AmendmentCategories.CSV,
  operation: (text) => {
    text = text.trim();
    const arr = text.split("\n").map((sp) =>
...
const selectFromCSV: AmendmentModule = {
  ...
  description:
    "Removes all columns with a header starting with TEMP (case sensitive)",
  category: AmendmentCategories.CSV,
  operation: (text) => {
    ...
    const indices = findIndices(
      header,
      (t) => t.trim() === "" || t.startsWith("TEMP"),
    );
    const filteredCSV = csvData.map((sl) =>
      sl.filter((_, index) => !indices.includes(index)),
    );
...
const extractNumberFromCsv: AmendmentModule = {
  ...
  description:
    'Given a CSV, replace all cells with the first "number" present in that cell, if any of them contain text. For example, if a cell contained "val = 333", this should change the cell to just 333.',
  category: AmendmentCategories.CSV,
  operation: (text) => {
...
const toGitBash: AmendmentModule = {
  ...
  operation: (text) => {
    ...
    return t2.startsWith("/") && t2.length >= 2
      ? t2[0] + t2[1].toLowerCase() + t2.slice(2)
      : t2;
  },
};
...
const toWSLPath: AmendmentModule = {
  ...
  operation: (text) => {
    ...
    return t2.startsWith("/mnt/") && t2.length >= 6
      ? t2.slice(0, 5) + t2[5].toLowerCase() + t2.slice(6)
      : t2;
  },
};
...
const thisPCFoldersAccessToFullPath: AmendmentModule = {
  ...
  repr: "Strip surrounding quotes",
  description:
    "Appends the full Windows path to folders in THIS PC: 3D Objects, Desktop, Documents, Downloads, Music, Pictures, Videos",
  category: AmendmentCategories.Paths,
  operation: (text) => {
    const targetFolders = [
      "3D Objects",
...
const extAmendmentModules: AmendmentModule[] = [
  "csv",
  "json",
  "md",
  "html",
...
const removeDuplicatesFromList: AmendmentModule = {
  ...
  repr: "dupe-remover",
  description:
    "Given a list of strings delimited by a newline, remove duplicates and preserve order otherwise",
  category: AmendmentCategories.Lists,
...
const csvToJSONRows: AmendmentModule = {
  ...
  description:
    "Convert a CSV to a JSON, each element is a row. Must have a header. The first actual row determines the types, which is inferred",
  category: AmendmentCategories.CSV,
  operation: (text) => {
    ...
    const types = lines[1]
      .split(",")
      .map((item) => (isNaN(Number(item)) ? "string" : "number"));
    const json = [];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      for (let j = 0; j < headers.length; j++) {
        obj[headers[j]] =
          types[j] === "number"
            ? nanToDefault(parseFloat(currentline[j]), 0)
            : currentline[j];
      }
...
const markdownShiftImageLinks: AmendmentModule = {
  ...
  repr: "md-subdir",
  description:
    "First line should contain the name of the directory, dubbed 'dir'. The rest of it should be markdown text. Changes all image links with format ![A][foo] to ![A][dir/foo]",
  category: AmendmentCategories.Markdown,
  operation: (markdown) => {
...
const removeLoneNewlines: AmendmentModule = {
  ...
  repr: "lone-newlines",
  description:
    "Removes any single newlines. Will not remove newlines where the next line starts with a -, ignoring whitespace. Will not remove newlines wrapped inside markdown code blocks.",
  category: AmendmentCategories.Strings,
  operation: (markdown) => {
    ...
    for (const item of splitList) {
      ...
      if (joinedList.length === 0 || in_code) {
        ...
        if (item.trim().startsWith("-") || item.trim().length === 0) {
          ...
          joinedList[joinedList.length - 1] =
            joinedList[joinedList.length - 1].trimEnd() +
            (joinedList[joinedList.length - 1].length > 0 ? " " : "") +
            item.trimStart();
        }
      }
    }
...
const setMinus: AmendmentModule = {
  ...
  repr: "set-minus",
  description:
    "If all elements are seperated by newlines, then return A \\ B, the split being the first line consisting of only dashes (---). This is the multi-set minus from SQL.",
  category: AmendmentCategories.Lists,
  operation: (markdown) => {
...
export const amendmentModules: AmendmentModule[] = [
  ...
  fixUnicodeEquations,
  align,
  pdfNumberedListToText,
  transposeMatrix,
  tsvToCsv,
  csvToTsv,
  tsvToJsonKeysBlankNull,
  csvToJsonKeysBlankNull,
  csvToJsonKeys,
  spaceToTabs,
  newTypeOldType,
  oldTypeNewType,
  stripLeadingSpaces,
  extractNumberFromCsv,
  removeLoneNewlines,
